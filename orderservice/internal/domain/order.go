...
var (
	...
	ErrOrderNotPending = errors.New("operation requires pending status")
	ErrOrderCanceled   = errors.New("cannot modify a canceled order")
...
func (o *Order) ApplyDiscount(percent int) error {
	...
	for i := range o.Items {
		discounted, err := o.Items[i].Price.ApplyDiscount(percent)
		if err != nil {
			return err
		}
